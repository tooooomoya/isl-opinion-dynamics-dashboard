...
- **betweenness centrality / λ<sub>2</sub>(algebraic connectivity)** — 無向化した
  **giant component上でのみ**計算する。`docs/journal.tex`の"Loss of the bridging role"
  節(「hub betweenness falls... measured on the undirected giant component」「directed
  full-graph betweenness is uninformative here because the hub is a near-pure in-degree
  sink」)の慣習に合わせている。有向グラフ全体でbetweennessを取っても、hubがほぼin-degree
  シンクである以上意味がないため。
...
| エンドポイント | パラメータ | 返り値 |
|---|---|---|
| `GET /api/summary` | — | `{seeds: [{seed, status, step, target, tag}], columns: [...], families: {base: [col_0..col_4]}}` — `logs/`で見つかった各seedにつき1行、これまでに見た`results.csv`の全カラムヘッダの和集合(`_0..4`サフィックスの列と`Q_sign`を除く)+選択可能な派生指標名、そして`FAMILY_RE`で検出したファミリーのグルーピング |
...
- **メトリクスグリッド** (`renderGrid`/`drawLineChart`/`drawFamilyChart`) — 選択した指標
  ...
  `apparentPolarization`、`unfollow`、`originalPostCount` — 2026-07-10変更、旧デフォルトは
  `docs/research-direction.md`の2-D outcome-plane6指標だった)。ホバーでクロスヘア+seed毎の
...
- **2-D outcome plane** (`renderPlane`) — 任意の2指標をx/y軸に選べる(セレクタ)、seed毎の
  ...
  Type-1/Type-2の構造分離×感情/意見分極という本来読みたい対象のためのパネル(この2軸が
  科学的に何を意味するかは`docs/research-direction.md`参照)。パネル上部に**時間スクラブ
...
- `result_dir()`のmtime-newestヒューリスティック(前述)は保証ではなく実質的な仮定であり、
  既知の失敗モードが1つある(手動でtouchされた古いフォルダ)。
